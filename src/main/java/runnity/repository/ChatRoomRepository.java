...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
    ...
    // 유저 그룹 채팅방 참가여부
    @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM ChatRoom c JOIN c.members m WHERE c.chatRoomId = :chatRoomId AND m.user.userId = :userId AND m.active = true")
