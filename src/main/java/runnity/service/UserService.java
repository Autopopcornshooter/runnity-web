...
public class UserService {
    ...
    public void updateUserInfo(UpdateUserInfoRequest request) {
        ...
        if (!request.getUsername().isBlank()) {  //입력값이 있을 경우
            user.setLoginId(request.getUsername());
        }
        if (!request.getPassword().isBlank()) { //입력값이 있을 경우
        ...
        log.info("User Login Id : {} password : {}", authenticatedUser().getLoginId(), authenticatedUser().getPassword());
